...
class NewDataController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $isDuplicateData = Data::where('nama_barang', $request->nama_barang)
                            ->where('kode_barang', $request->kode_barang)
                            ->exists();
        $isDuplicateNamkod = Namkod::where('nama_barang', $request->nama_barang)
        ->where('kode_barang', $request->kode_barang)
        ->exists();

        if ($isDuplicateData || $isDuplicateNamkod) {
            return redirect()->route('data.new.create')->with('error', 'Nama Barang dan Kode Barang sudah ada dalam database.');
        }

        try {
            Data::create($request->only(['nama_barang', 'kode_barang']));
        Namkod::create($request->only(['nama_barang', 'kode_barang']));
